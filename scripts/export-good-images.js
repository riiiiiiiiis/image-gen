...
async function downloadWithConcurrency(downloadTasks, concurrency = 5) {
  ...
  async function downloadNext() {
    ...
    try {
      ...
      console.log(`✅ Downloaded ${currentIndex + 1}/${downloadTasks.length}: ${task.filename}`);
...
async function main() {
  console.log('🚀 Starting export of good images...');
  ...
  console.log('🔍 Querying database for good images...');
  ...
  if (!entries || entries.length === 0) {
    console.log('ℹ️ No good images found in database');
    return;
  }
  
  console.log(`📊 Found ${entries.length} good images to export`);
